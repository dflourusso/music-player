...
function currentPlayingSoundSelector(state: RootState): CurrentPlayingSoundType | undefined {
...
type currentPlayingSoundSelectorType = ReturnType<typeof currentPlayingSoundSelector>
...
export function* unloadPrevious() {
  const audioSound = (yield select(audioSoundSelector)) as audioSoundSelectorType
  if (audioSound) {
    try {
...
export function* initSound(action: any) {
  ...
  const audioSound = (yield call([audio, audio.createAudioSound], action.payload)) as Audio.Sound
  if (audioSound) {
    yield put(
...
export function* back() {
  const currentPlayingSound = (yield select(currentPlayingSoundSelector)) as currentPlayingSoundSelectorType
