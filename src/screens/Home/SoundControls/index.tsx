...
import { Container, Content, ProgressContainer, ProgressLeft, ProgressRight } from './styles'
...
const SoundControls: React.FC = () => {
  ...
  return (
    <Container>
      <ProgressContainer>
        <ProgressLeft>{millisToMinutesAndSeconds(progress?.positionMillis ?? 0)}</ProgressLeft>
        <Slider
        ...
        />
        <ProgressRight>{millisToMinutesAndSeconds(progress?.durationMillis ?? 0)}</ProgressRight>
      </ProgressContainer>
      <Content>
